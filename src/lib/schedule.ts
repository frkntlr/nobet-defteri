...
function preferredShiftForFill(
  ...
  if (employee.gender === "male") {
    ...
    if (isNightSpecialist(employee) || (specialistCovers && !isMorningSpecialist(employee))) {
...
function pickConvertShift(
  ...
  const ranked = pool
    ...
    .map((person) => {
      ...
      if (to === "morning" && maleMorningHasSpecialist(date, pool, cells) && !isMorningSpecialist(person)) return null;
