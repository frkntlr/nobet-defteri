...
function preferredShiftForFill(
  ...
  if (employee.gender === "male") {
    ...
    if (mornings < maleFloor(settings, "morning")) return "morning";
    ...
    while (i < days.length) {
      ...
      if (runLen < need && !continuesOutsideMonth(person, days, i, j - 1, settings)) {
        for (let k = i; k < j; k += 1) {
          ...
          if (wouldMissTarget(person, days, cells, settings)) continue;
          if (wouldBreakMaleCoverage(person, date, cells, active, settings)) {
            const needShift = cell.shift;
            const pool = (person.gender === "male" ? males : active).filter((e) => e.id !== person.id);
            const other =
              pickFillCandidate(
                date,
                needShift,
            ...
            if (other) assignFill(other, date, needShift, cells, fills);
            else if (needShift === "night" && person.gender === "male") {
              convertMorningToNight(date, "male", pool, cells, fills, days, leaves, manuals, settings, pairs);
            }
            if (wouldBreakMaleCoverage(person, date, cells, active, settings)) continue;
          }
          cells[key] = { ...cell, shift: "off", source: "adjust" };
        }
      }
